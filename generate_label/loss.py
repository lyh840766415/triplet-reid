...
def batch_loss(margin,dists):
	diff = tf.abs(tf.subtract(margin,dists))
	diff = tf.reduce_sum(diff,0)
	return diff
	
	
#TODO batch_accuracy & batch_recall
def batch_accuracy(dists):
	
	return dists
	
def batch_recall(dists):
	return dists
	
def batch_accuracy_recall(dists,same_label,batch):
	same_label = tf.cast(same_label,tf.bool)
	_,indices = tf.nn.top_k(dists, 100)
	
	top1 = indices-1
	top1 = tf.cast(top1,tf.bool)
	top1 = tf.logical_not(top1)


	acc = tf.logical_and(same_label,top1)
	acc = tf.cast(acc,tf.float32)
	

	top1 = tf.cast(top1,tf.int32)

	top2 = indices-2
	top2 = tf.cast(top2,tf.bool)
	top2 = tf.logical_not(top2)
	top2 = tf.cast(top2,tf.int32)

	top3 = indices-3
	top3 = tf.cast(top3,tf.bool)
	top3 = tf.logical_not(top3)
	top3 = tf.cast(top3,tf.int32)

	top4 = indices-4
	top4 = tf.cast(top4,tf.bool)
	top4 = tf.logical_not(top4)
	top4 = tf.cast(top4,tf.int32)

	top4 = top1+top2+top3+top4
	top4 = tf.cast(top4,tf.bool)
	recall = tf.logical_and(same_label,top4)
	recall = tf.cast(recall,tf.float32)
	acc = tf.reduce_sum(acc, axis = 1)
	recall = tf.reduce_sum(recall, axis = 1)/4
